...
async function employeeEvaluationReport() {
    const tableEvaluation = document.getElementById('evaluation_report');

    const res = await fetch(`${CONFIG.BASE_URL}employee/get_evaluation_report/${employee_id}`);
    const result = await res.json();
    if (result) {
        evaluations = JSON.parse(result.evaluation_report_data);
    } 
    document.getElementById('submit').disabled = result?.employee_role_id != 1;
    tableEvaluation.innerHTML = evaluations.map((group, groupIndex) => (`
            ...
            ${group.questions.map((question, questionIndex) => (`
                <tr>
                    ...
                    <td class="p-0">
                        <strong>
                            <select class="form-control rating-input" data-group-index="${groupIndex}" ${result?.employee_role_id != 1 ? 'disabled': ''} data-question-index="${questionIndex}">
                                <option value=""></option>
                    ...
                    <td class="p-0">
                        <div style="display: grid;">
                            <textarea class="form-control textArea" placeholder="Remarks..." data-group-index="${groupIndex}" ${result?.employee_role_id != 1 ? 'disabled': ''} data-question-index="${questionIndex}">${question.remarks || ''}</textarea>
                        </div>
                    </td>
                </tr>
            `))
...
document.getElementById('submit').addEventListener('click', async () => {
    ...
    const res = await fetch(`${CONFIG.BASE_URL}employee/store_evaluation_report/${employee_id}`, {
        method: 'POST',
        body: formData,
    });
    const result = await res.json();
